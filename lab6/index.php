...
define('DB_NAME', 'u82408');   // ваш логин = имя БД
define('DB_USER', 'u82408');   // ваш логин
define('DB_PASS', '7505322'); // ваш пароль
...
// Возвращает массив ['uid' => ..., 'login' => ...] или false
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // ── Авторизованный пользователь: грузим данные из БД ──────
    if ($sessionUser) {
        try {
            $db   = get_db();
            $stmt = $db->prepare(
            ...
            $messages['error_hint'] = 'Ошибка загрузки данных: ' . $e->getMessage();
        }

        // При авторизованном входе ошибки из cookies не нужны —
        // но если они есть (после неудачного POST), всё равно покажем.
        $fieldMap = [
        ...
        foreach ($fieldMap as $field => $cookieName) {
            if (!empty($_COOKIE[$cookieName])) {
                ...
                $anyErr = true;
                // Значение из POST-попытки (временная кука) перезаписывает БД-значение
    ...
    // ── Неавторизованный пользователь: данные из cookies ──────
    } else {

        // Сообщение об успешном сохранении + показ логина/пароля (один раз)
...
// ══════════════════════════════════════════════════════════════
//  POST — валидация → cookies → redirect
// ══════════════════════════════════════════════════════════════
$post        = $_POST;
$hasErr      = false;
$sessionUser = get_session_user();
...
if ($hasErr) {
    // Если авторизован — сохраняем временные значения для отображения в форме
    if ($sessionUser) {
        // Значения уже выставлены как set_temp_cookie выше
    }
...
try {
    ...
    if ($sessionUser) {
        ...
        // Перезаписываем языки: удаляем старые, вставляем новые
    ...
    // ── Не авторизован → INSERT + генерация логина/пароля ─────
    ...
    } while ($check->fetch()); // гарантируем уникальность
    ...
    $plainPass = str_shuffle($plainPass); // 8 символов, буквы+цифры

    $passHash  = md5($plainPass); // md5 как в примере преподавателя
...
// Передаём логин/пароль через куки (показать один раз на GET)
setcookie('save',      '1',        0, '/');
